package com.weiteng.image.luban;
...
public class Luban {
    ...
    private void checkNotNull(Object obj, String s) {
        if (obj == null) {
    ...
    private byte[] thirdCompress(@NonNull File file) {
        ...
        if (scale <= 1 && scale > 0.5625) {
            if (height < 1664) {
                if (file.length() / 1024 < 150) return getOriginalBytes(file);
            ...
            if (height < 1280 && file.length() / 1024 < 200) return getOriginalBytes(file);
    ...
    private byte[] getOriginalBytes(File file) {
        try {
